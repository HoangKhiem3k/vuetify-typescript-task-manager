...
<template>
    <form @submit.prevent="submit">
        <v-text-field v-model="description.value.value" :counter="10" :error-messages="description.errorMessage.value"
            label="Description" />
        <v-checkbox v-model="isReminderSet.value.value" :error-messages="isReminderSet.errorMessage.value" value="1"
            label="Reminder" type="checkbox" />
        <v-select v-model="select.value.value" :items="priorities" :error-messages="select.errorMessage.value"
            label="Priority" />
